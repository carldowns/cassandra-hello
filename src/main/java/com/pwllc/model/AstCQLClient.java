...
import com.pwllc.rest.CassResult;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.Map;

import static com.pwllc.model.ModelConstants.*;
...
public class AstCQLClient {
    ...
    public void init() {
        ...
        String kname = keyspace.getKeyspaceName();
        String cname = context.getClusterName();
        //keyspace = context.getEntity();

        EMP_CF = ColumnFamily.newColumnFamily(
                EMP_CF_NAME,
                IntegerSerializer.get(),
                StringSerializer.get(), // added by CBD adding a default serializer
    ...
    public void insertDynamicProperties(int id, Map<String,String> updates) {
        MutationBatch m = keyspace.prepareMutationBatch();

        ColumnListMutation<String> clm = m.withRow(EMP_CF, id);
        for (String key : updates.keySet()) {
            clm.putColumn(key, updates.get(key), null);
        }
    ...
    public CassResult read(int empId) {
        logger.debug("read()");
        try {
            ...
            CassResult cassResult = new CassResult(Integer.toString(empId), "reading");
            for (Row<Integer, String> row : result.getResult().getRows()) {
                ...
                cassResult.setProperty(COL_NAME_FIRST_NAME, cols.getStringValue(COL_NAME_FIRST_NAME, null));
                cassResult.setProperty(COL_NAME_LAST_NAME, cols.getStringValue(COL_NAME_LAST_NAME, null));
            }
            return cassResult;
